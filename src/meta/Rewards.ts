...
export function calculateMissionGloryReward(
  ...
  const value = (base + waveBonus) * difficultyMultiplier * resultMultiplier * goldEarnedMultiplier;
  return {
    base,
    waveBonus,
...
export function calculateRunBonusGlory(won: boolean, goldEarnedMultiplier: number): number {
  const base = won ? BALANCE_CONFIG.glory.runWinBonus : BALANCE_CONFIG.glory.runLossBonus;
  return Math.max(0, Math.round(base * goldEarnedMultiplier));
}
